<div class="header">
    <nav class="pink">
        <div class="container">
            <div class="nav-wrapper">
                <div class="col s12">
                    <a href="#" class="brand-logo">Test</a>
                    %{--<span class="right">Exit Test</span>--}%
                    <a href="#" data-activates="mobile-demo" class="button-collapse"><i class="material-icons">menu</i></a>
                    <ul class="right hide-on-med-and-down">
                        <li><a hred="#">Home</a></li>
                        <li><a href="#">Test</a></li>
                        <li><a href="#" data-activates="user" class="waves-effect waves-teal dropdown-button authentication" style="display:none"><i class="material-icons left">perm_identity</i>Hi Akshay</a></li>
                    </ul>

                    <ul class="right dropdown-content" id="user">
                        <li><a href="login" class=" center pink white-text">Logout</a> </li>
                    </ul>

                    <ul class="side-nav" id="mobile-demo">
                        <li><a href="#" class="waves-effect waves-teal flow-text center-align">Home</a></li>
                        <li><div class="divider"></div></li>
                        <li><a href="#" class="waves-effect waves-teal flow-text center-align">Test</a></li>
                        <li><div class="divider"></div></li>
                        <li><a href="#" class="waves-effect waves-teal flow-text center-align authentication" style="display:none">Akshay</a></li>
                    </ul>
                </div>
            </div>
        </div>
    </nav>
</div> <!-- end header-->